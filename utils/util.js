...
function cutstr(str, len,flag) {
        ...
        for (var i = 0; i < str_len; i++) { 
            var a = str.charAt(i);  //拆分成单个字符，i为字符的索引
            str_length++;
            if (escape(a).length > 4) { //escape()将字符转换为16进制，中文4个16进制，英文2个16进制
                //中文字符的长度经编码之后大于4  
                str_length++;   //中文占两个字节所以++
            }
            str_cut = str_cut.concat(a);//添加字符串到指定数组
            if (str_length >= len) {
              if (flag == 0){   //flage==0将会加省略号
...
function GetUrlFileName(url,domain) {    
    var filename = url.substring(url.lastIndexOf("/") + 1);     //+1 说明不包含这个/，只定义开始截取的位置，没有定义结束的位置，说明只截取后面/分割的最后一个段
    if (filename == domain || filename =='')        //url传递过来就是空或者是本网站后端域名
    {
        filename="index";
    }
    else
